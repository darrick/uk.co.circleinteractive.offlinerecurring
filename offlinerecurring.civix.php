 ...
 * @return array(string)
 */
function _offlinerecurring_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_offlinerecurring_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
...
function _offlinerecurring_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_offlinerecurring_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _offlinerecurring_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _offlinerecurring_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 */
function _offlinerecurring_civix_glob($pattern) {
  $result = glob($pattern);
 ...
 *    filled for you)
 */
function _offlinerecurring_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _offlinerecurring_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
 ...
 */
function _offlinerecurring_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _offlinerecurring_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _offlinerecurring_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _offlinerecurring_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _offlinerecurring_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _offlinerecurring_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
